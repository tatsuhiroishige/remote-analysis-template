...
**MCP workflow:**
```
open_file("macro/analysis.C")
insert_after(<line_after_last_histogram>, "TH1D* h_newVar = ...")
insert_after(<line_in_event_loop>, "h_newVar->Fill(variable);")
commit_edit("macro/analysis.C", "Added h_newVar histogram")
```
...
```
open_file("macro/analysis.C")
replace("pt > 0.5", "pt > 1.0")
commit_edit("macro/analysis.C", "Updated pt cut to 1.0")
```
...
6. **Execute** edit: `replace()`, `insert_after()`, or `delete_lines()` + `bulk_insert()`
...
- Changing core logic (formulas, etc.)
