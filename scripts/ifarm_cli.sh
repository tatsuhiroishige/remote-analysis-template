#!/bin/bash
# ifarm_cli.sh — Local-pane unified control (fallback for MCP)
#
# Architecture:
#   All send/capture operations go through LOCAL_PANE.
#   LOCAL_PANE runs SSH into remote (with or without remote tmux attach).
#   _rtmux is used only for parallel sessions (term-*) and remote-only queries.
#
# Usage: ./scripts/ifarm_cli.sh <command> [args...]
...
LOCAL_PANE="myserver:view.0"                 # Local tmux pane (<session>:<window>.<pane>)
MAIN="$SESSION:ide.0"                        # Remote main pane
WORKDIR="/home/user/analysis"                # Remote working directory

# --- Internal: remote tmux command (SSH only) ---
_rtmux() {
...
case "$1" in
  ...
  setup)
    local_session="${LOCAL_PANE%%:*}"
    if ! tmux has-session -t "$local_session" 2>/dev/null; then
      echo "Error: local tmux session '$local_session' not found" >&2; exit 1
    fi
  ...
  findlines)
    ...
    ssh "$REMOTE" "cd $WORKDIR && grep -n '$3' $2" 2>/dev/null | grep -v '^Loading\|WARNING\|requirement'
    ;;
